...
const Desktop: FC<DesktopProps> = (props) => {
	...
	const handleFocus = (indexToMove: number) => {
		...
		setWindowOrder((prevOrder) => [
			...prevOrder.filter((x) => x !== windowToFocus),
			windowToFocus,
		]);
	};
					...
					{windows.map((item, index) => {
						...
						if (item.type === "folder") {
							...
							size = {
								...
								height:
									Math.min(684, maxWidth) *
										(item.attachment.height / item.attachment.width) +
									44,
							};
